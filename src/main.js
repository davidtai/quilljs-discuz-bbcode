...
var allowimgurl = true;
var allowparseurl = true;
...
function preg_replace(search, replace, str, regswitch) {
    regswitch = !regswitch ? 'ig' : regswitch;
    var len = search.length;
    for (var i = 0; i < len; i++) {
        var re = new RegExp(search[i], regswitch);
...
function parseurl(str, mode, parsecode) {
    ...
    str = str.replace(
        /([^>=\]"'\/]|^)((((https?|ftp):\/\/)|www\.)([\w\-]+\.)*[\w\-\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!]*)+\.(swf|flv))/ig,
        '$1[flash]$2[/flash]'
    );
    str = str.replace(
        /([^>=\]"'\/]|^)((((https?|ftp):\/\/)|www\.)([\w\-]+\.)*[\w\-\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!]*)+\.(mp3|wma))/ig,
        '$1[audio]$2[/audio]'
    );
    str = str.replace(
        /([^>=\]"'\/@]|^)((((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|qqdl|synacast):\/\/))([\w\-]+\.)*[:\.@\-\w\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&;~`@':+!#]*)*)/ig,
    ...
    if (parsecode) {
        for (var i = 0; i <= codenum; i++) {
            str = str.replace('[\tDISCUZ_CODE_' + i + '\t]', codehtm[i]);
        }
    }
    return str;
}
...
function getoptionvalue(option, text) {
    var re = new RegExp(option + '(\s+?)?\=(\s+?)?["\']?(.+?)(["\']|$|>)', 'ig');
...
function fetchoptionvalue(option, text) {
    var position, delimiter, delimchar, delimloc;
...
function simpletag(options, text, tagname, parseto) {
    ...
    if (options && options.indexOf('style') > -1) {
        text = fonttag(options, text, tagname);
    }
...
function atag(aoptions, text) {
    ...
    var href = getoptionvalue('href', aoptions);
...
function codetag(text, br) {
    ...
    codehtm[codenum] = '[code]' + text + '[/code]';
    return '[\tDISCUZ_CODE_' + codenum + '\t]';
}
...
function dstag(options, text, tagname) {
    ...
    if (in_array(tagname, ['div', 'p'])) {
        var align = getoptionvalue('align', options);
...
function fonttag(fontoptions, text, tagname) {
    var prepend = '';
    var append = '';
    var tags = {
        'font': 'face=',
    ...
    for (var bbcode in tags) {
        var optionvalue = fetchoptionvalue(tags[bbcode], fontoptions);
        if (optionvalue) {
    ...
    return pend['prepend'] + recursion(tagname, text, 'fonttag') + pend['append'];
}
...
function ptag(options, text) {
    ...
    var style = getoptionvalue('style', options);
    style = preg_replace(
        ['line-height\\\s?:\\\s?(\\\d{1,3})px', 'text-indent\\\s?:\\\s?(\\\d{1,3})em', 'text-align\\\s?:\\\s?(left|center|right)'],
        '', style
    );
...
function bbcode2html(str) {
    ...
    if (allowbbcode) {
        ...
        str = str.replace(/\[size=(\d+?)\]/ig, function ($1, $2) {
            return fontsizelist[$2] ? '<span style="font-size:' + fontsizelist[$2] + '">' : '<span>';
        });
        str = str.replace(/\[size=(\d+(\.\d+)?(px|pt)+?)\]/ig, '<span style="font-size:$1">');
        str = str.replace(/\[font=([^\[\<\=]+?)\]/ig, function ($1, $2) {
            $2 = $2.replace(/&quot;?/g, '').replace(/[,\s]+$/g, '');
            return $2 ? '<span style="font-family:' + $2 + '">' : '<span>';
        });
        str = str.replace(/\[align=([^\[\<\=]+?)\]/ig, '<div style="text-align:$1">');
        ...
        var re = /\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*([\s\S]+?)\s*\[\/table\]/ig;
        for (var i = 0; i < 4; i++) {
        ...
        if (allowimgcode) {
            ...
            str = str.replace(/\[attachimg\](\d+)\[\/attachimg\]/ig, function ($1, $2) {
                ...
                var width = elem('image_' + $2).getAttribute('cwidth');
                if (!width) {
                    re = /cwidth=(["']?)(\d+)(\1)/i;
                    var matches = re.exec(elem('image_' + $2).outerHTML);
                    if (matches != null) {
                        width = matches[2];
                    }
                }
                return '<img src="' + elem('image_' + $2).src + '" border="0" aid="attachimg_' + $2 + '" width="' + width + '" alt="" />';
            });
...
function parsecode(text) {
    codenum++;
    codehtm[codenum] = '<div class="blockcode"><blockquote>' + htmlspecialchars(text) + '</blockquote></div>';
    return '[\tDISCUZ_CODE_' + codenum + '\t]';
}
...
function parsestyle(tagoptions, prepend, append) {
    ...
    var re = /^(?:\s|)color:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)(;?)/i;
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (match != null) {
            var opnvalue = match[searchlist[i][3]];
            if (searchlist[i][4] == 'size') {
                opnvalue = fontsizelist[opnvalue];
            }
...
function parsetable(width, bgcolor, str) {
    ...
    return '<table ' + (width == '' ? '' : 'width="' + width + '" ') + 'class="t_table"' + (bgcolor === undefined ? '' : ' style="background-color: ' + bgcolor + '"') + simple + '>' + str + '</table>';
}
